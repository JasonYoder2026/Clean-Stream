
# Login

When you access the portal, you’ll be prompted with the **Log In** screen.  
Here, you can enter your **username** and **password**, then tap **Log In** to access your account.  
This is the same login you use for the mobile app, after onboarding has been complete and Jake has set up your admin account.<br>
<img width="1220" height="800" alt="image" src="https://github.com/user-attachments/assets/7d9bde8a-c731-4dda-8e86-b93ac80ee55c" />
<br>
---
# Home Dashboard

After logging in, you’ll be directed to the home page.  
Here you will see widgets displaying the daily revenue and a graph of the last 30 days income.<br>
<img width="1458" height="752" alt="image" src="https://github.com/user-attachments/assets/ed6a6c2b-cce7-4e9b-92e5-42a28d894465" />
" />


---

# Refund 
Clicking the refunds tab on the side bar brings you here. Here you can  
approve or deny refunds. <br>
<img width="1837" height="881" alt="image" src="https://github.com/user-attachments/assets/e24ada28-19b2-4af1-a439-1de242a9573e" />
<br>
By clicking Respond, you can approve or deny the request and add a note for your reasoning. The customer will automatically get an email with the details.<br>

<img width="567" height="633" alt="image" src="https://github.com/user-attachments/assets/ac247963-3d24-41b8-9ad9-6a2b75a271b9" />
<br>

---

# Maintenance
Tapping the Maintenance tab brings you here. On this page you can view maintenance requests from users and delete them as they're taken care off. <br>
<img width="1907" height="901" alt="image" src="https://github.com/user-attachments/assets/d3e1051a-c41d-4a7f-a9ad-5a8947973950" />
<img width="1907" height="907" alt="image" src="https://github.com/user-attachments/assets/421fadf4-1585-464c-87e3-eaf5be2f42cd" />


---

# Locations
Tapping the Locations tab brings you here. On this page you can adjust machines for locations you are assigned to <br>
<img width="1877" height="787" alt="image" src="https://github.com/user-attachments/assets/b2cae393-65a3-4aaf-9b84-06d791a81384" />

---
# Employees

This page is only accessible by an owner. Here, they can manage employees and which locations employees are assigned to.<br>
<img width="1902" height="822" alt="image" src="https://github.com/user-attachments/assets/31e889cc-a8e7-4e84-be4e-56389a885660" />
